 ...
 */
export const saveMessagesToStorage = (conversationId: string, messages: Message[]): void => {
  try {
 ...
 */
export const loadMessagesFromStorage = (conversationId: string): Message[] => {
  try {
    const key = `${MESSAGES_PREFIX}${conversationId}`;
 ...
 * @param conversationId - The conversation ID
 */
export const deleteMessagesFromStorage = (conversationId: string): void => {
  try {
    const key = `${MESSAGES_PREFIX}${conversationId}`;
 ...
 */
export const addMessageToStorage = (conversationId: string, message: Message): void => {
  try {
 ...
 */
export const hasMessages = (conversationId: string): boolean => {
  const key = `${MESSAGES_PREFIX}${conversationId}`;
 ...
 */
export const getAllV2ConversationIds = (): string[] => {
 ...
 */
export const cleanupOrphanedMessages = (validConversationIds: string[]): number => {
  let cleanedCount = 0;
