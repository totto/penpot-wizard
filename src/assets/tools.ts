import { ProjectData, FunctionTool, RagTool, UserData } from '@/types/types';
import { z } from 'zod';
import { $userData } from '@/stores/penpotStore';
import { $projectData } from '@/stores/penpotStore';

interface GetPenpotDataResponse {
  activeUserInfo?: UserData;
  activeProjectInfo?: ProjectData;
}
...
export const functionTools: FunctionTool[] = [
  {
    ...
    function: async ({ includeActiveUserInfo, includeActiveProjectInfo }) => {
      const response: GetPenpotDataResponse = {};

      if (includeActiveUserInfo) {
        response.activeUserInfo = $userData.get();
      }
      if (includeActiveProjectInfo) {
        response.activeProjectInfo = $projectData.get();
      }
      return response;
    },
  }
