import { FunctionTool, ClientQueryType, GetCurrentPagePayload } from '@/types/types';
import { z } from 'zod';
...
export const functionTools: FunctionTool[] = [
  {
    ...
    function: async () => {
      ...
      if (response.success && response.payload) {
        const payload = response.payload as GetCurrentPagePayload;
        
        // Curate shapes array
        const curatedShapes = payload.shapes.map(shape => 
          curateShapeOutput(shape as Record<string, unknown>)
        );
  ...
  },
  {
    ...
    function: async ({ category }) => {
      ...
      if (category) {
        ...
        if (presets[categoryUpper as keyof typeof presets]) {
          return {
            success: true,
            category: categoryUpper,
            presets: presets[categoryUpper as keyof typeof presets],
            total: presets[categoryUpper as keyof typeof presets].length,
          };
        } else {
          return {
            success: false,
